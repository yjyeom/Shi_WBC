...
\section{SHI'S WHITE-BOX CRYPTOGRAPHY}
In 2019, Shi et al. proposed a light-weight white-box encryption scheme
for light-weight embedded devices including mobile phones and navigating systems. 
We denote this scheme by LW-WBES.
In fact, LW-WBES is a block cipher with the following features:
\begin{itemize}
\item 120 bit block size (input/output size)
\item 16(default), 10(aggressive), or 32(conservative) rounds depending on the security level
\item variants of Feistel network 
\item two types of keys: black-box key and white-box key 
\end{itemize}
