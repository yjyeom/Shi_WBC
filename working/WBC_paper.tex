...
of standard ciphers, Shi et al. propose a new cipher which seems to be secure 
against white-box attack context.
Their design stretergies can be summarized as:
\begin{itemize}
\item The scheme has the secret components
based on the Feistel network, which protect the secret keys from
white-box attacks including DCA and DFA.
\item Since components of three different size (4, 5, 6-bit) are integrated,
it is hard to mount the structural analysis.
\item The secret components can be reused in each round for saving memory usage
in light-weight devices.
\item The scheme does not require external encodings
and obfuscation techniques. 
\end{itemize}
We will show that the goal of design ratinals cannot be satisfied
...
Here, we describe the encryption process of16 round version.
120-bit plaintext $PT$ is input $(L,R)$ for the Feistel network divided into
5-bit variables as:
\[
...
In each round, the round function $F: GF(2)^{60} \to GF(2)^{72}$
consumes 72-bit black-box round key $rk$.
